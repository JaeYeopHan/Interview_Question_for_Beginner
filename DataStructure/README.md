...
가장 기본적인 자료구조인 `Array` 자료구조는, 논리적 저장 순서와 물리적 저장 순서가 일치한다. 따라서 `인덱스`(index)로 해당 원소(element)에 접근할 수 있다. 그렇기 때문에 찾고자 하는 원소의 인덱스 값을 알고 있으면 `Big-O(1)`에 해당 원소로 접근할 수 있다. 즉 **random access** 가 가능하다는 장점이 있는 것이다. 배열은 원소의 조회, 수정, 삭제 모두 O(1)인 자료구조여서 가장 많이 쓰인다. 

배열은 생성할 때 정적 할당(static allocation) 그 크기를 지정해줘야 한다는 단점이 있다. 이를 보완한 것이 `ArrayList` 혹은 `Vector`라고 불리는 자료형들이다. 이들은 동적으로 할당(dynamic allocation)이 되어 길이가 가변적이다. 또, 원소 사이에 빈 공간을 허용하지 않는다는 특징을 갖는다. 따라서 기본적인 배열과 다르게 삽입과 삭제가 `Big-O(n)`의 시간복잡도가 걸린다.

### Linked List

Linked list는 임의접근이 불가능하며 각각의 원소들은 자기 자신 다음에 어떤 원소인지만을 기억하고 있다. 따라서 이 부분만 다른 값으로 바꿔주면 삭제와 삽입을 O(1) 만에 해결할 수 있는 것이다.
