...
Swift는 객체지향, 함수형, 프로토콜 지향 패러다임을 지원하는 멀티 패러다임 언어이다.

* 객체지향 프로그래밍 패러다임
* 함수형 프로그래밍 패러다임
  * 함수를 일급객체 (first-class citizen)로 다룬다. 즉 함수를 전달인자로 전달할수 있고
    리턴 값으로도 사용할 수 있고 변수에 담을 수 도 있다.
  * 순수함수 = 함수 스코프 밖의 변수를 건들이지 않는다. 함수에 전달된 인자값만 영향을 준다.
  * 참조투명성 = 함수에 같은 값의 인풋을 준다면 아웃풋은 항상 같다
* 프로토콜 지향 프로그래밍 패러다임
  * 특정 프로토콜을 채택하면 요구하는 기능을 구현해야한다
